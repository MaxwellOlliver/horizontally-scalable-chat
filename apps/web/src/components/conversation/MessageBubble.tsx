    ...
    <div className={`flex ${mine ? 'justify-end' : 'justify-start'}`}>
      <div
        className={`max-w-[78%] rounded-2xl px-3.5 py-2 ${
          mine ? 'border border-accent/20 bg-accent/12' : 'bg-panel-2'
        } ${failed ? 'border-danger/40' : ''}`}
      >
        <p className="whitespace-pre-wrap break-words text-[13.5px] leading-relaxed text-fg">
          {message.body}
        </p>
        <div className="mt-1 flex items-center justify-end gap-1.5">
          <span className="font-mono text-[10px] text-fg-faint">{clockTime(message.createdAt)}</span>
